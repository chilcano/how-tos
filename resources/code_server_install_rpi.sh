...
./mkcert -install
./mkcert rpi-vscs localhost 127.0.0.1 ::1
...
sed -i.bak 's/cert: false/cert: vscs.rpi+3.pem/' ~/.config/code-server/config.yaml
echo -e 'cert-key: vscs.rpi+3-key.pem' >> ~/.config/code-server/config.yaml
