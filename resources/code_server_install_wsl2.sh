...
mkcert -install
if [[ -f vscs-wsl2.pem ]] && [[ -f vscs-wsl2-key.pem ]]; then
  ...
  cp localhost+2.pem ~/vscs-wsl2.pem
  cp localhost+2-key.pem ~/vscs-wsl2-key.pem
fi
printf "\n\n"
...
printf "\t code-server --disable-telemetry --bind-addr 127.0.0.1:8443 --cert vscs-wsl2.pem --cert-key vscs-wsl2-key.pem   # uses created TLS certs \n\n"
