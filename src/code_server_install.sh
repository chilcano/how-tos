...
unset _VSCS_VER _ARCH VSCS_VER_LATEST VSCS_VER VSCS_DOWNLOAD_URL VSCS_BUNDLE_NAME _TLS
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --arch*|-a*)
      ...
      ;;
    --tls*|-t*)
      if [[ "$1" != *=* ]]; then shift; fi # values : no, yes (by default)
      _TLS="${1#*=}"
      ;;
    --help|-h)
...
./mkcert -install
./mkcert vscs.ubuntu $HOSTNAME localhost 127.0.0.1 ::1
mv vscs.* ~/
printf "\n"

echo ">> Tweaking '~/.config/code-server/config.yaml' \n"
if [ -z ${_TLS+x} ] || [ "${_TLS}" != "no" ]; then 
  echo "=> TLS is going to be enabled."
  sed -i.bak 's/^bind-addr: .*$/bind-addr: 0.0.0.0:8443/' ~/.config/code-server/config.yaml
  sed -i.bak 's/cert: false/cert: vscs.ubuntu+4.pem/' ~/.config/code-server/config.yaml
  echo -e 'cert-key: vscs.ubuntu+4-key.pem' >> ~/.config/code-server/config.yaml
elif [ "${_TLS}" = "no" ]; then
  echo ">> TLS is going to be disabled."
  sed -i.bak 's/^bind-addr: .*$/bind-addr: 0.0.0.0:8080/' ~/.config/code-server/config.yaml
fi
echo -e 'disable-telemetry: true\n' >> ~/.config/code-server/config.yaml
printf "\n"

printf ">> The final '~/.config/code-server/config.yaml' is: \n"
echo "-----------------------------------------------"
cat ~/.config/code-server/config.yaml
echo "-----------------------------------------------"

printf ">> Follow next steps if only TLS has been enabled.\n"
printf ">>   Trust on the Root CA crt generated by 'mkcert'\n"
printf ">>   You have to install it in your browser as trusted CA and add '$HOSTNAME 192.168.1.44' in your '/etc/hosts' file.\n"
...
AWS_TOOLKIT_VSIX_URL=$(curl -s https://api.github.com/repos/aws/aws-toolkit-vscode/releases/latest | jq -r -M '.assets[].browser_download_url' | grep 'vsix$')
