/*
The S3 bucket used for CloudFront distribution is allowing more permissions than needed to the 
CloudFront Origin Identity. An adversary could take advantage of such wide permissions to 
perform unauthorized actions over the S3 bucket objects.

The CloudFront Origin Identity should have read-only permissions over the S3 bucket objects. 
The statement actions = ["s3:GetObject"] was added to the S3 bucket policy. This action 
prevents CloudFront Origin Identity from performing any other operation over the S3 bucket 
objects.
*/
